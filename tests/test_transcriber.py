...
def test_transcribe_handles_missing_audio_file(tmp_path):
    # File that doesn't exist — remove should not raise
    p = tmp_path / "audio.wav"
    p.write_bytes(b"RIFF data")
    audio_path = str(p)
    ...
    with patch("transcriber.OpenAI") as MockOpenAI:
        ...
        with patch("transcriber.os.remove", side_effect=FileNotFoundError):
            from transcriber import transcribe
    ...
    assert result == "Text"  # Should not raise, should return text
